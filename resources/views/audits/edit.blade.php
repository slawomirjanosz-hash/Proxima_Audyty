                                                ...
                                                <tr data-field-row="1" data-parent-token="{{ $field['parent_token'] }}" data-show-when="{{ $field['show_when'] }}" class="{{ $field['parent_token'] !== '' ? 'is-dependent-row' : '' }}">
                                                    ...
                                                    <td>
                                                        @if($field['kind'] === 'boolean')
                                                            <select data-audit-section="{{ $section->id }}" data-field-token="{{ $field['key'] }}" data-field-kind="{{ $field['kind'] }}" class="formula-source" name="section_payload[{{ $section->id }}][rows][{{ $rowIndex }}][value]">
                                                                <option value="">—</option>
                                                        ...
                                                        @elseif($field['kind'] === 'text')
                                                            <input data-audit-section="{{ $section->id }}" data-field-token="{{ $field['key'] }}" data-field-kind="{{ $field['kind'] }}" class="formula-source" type="text" name="section_payload[{{ $section->id }}][rows][{{ $rowIndex }}][value]" value="{{ $value }}" placeholder="Wpisz opis">
                                                        @else
                                                            <input data-audit-section="{{ $section->id }}" data-field-token="{{ $field['key'] }}" data-field-kind="{{ $field['kind'] }}" class="formula-source" type="number" step="any" name="section_payload[{{ $section->id }}][rows][{{ $rowIndex }}][value]" value="{{ $value }}">
                                                        @endif
                                                    </td>
                                                    <td>
    ...
    <script>
        ...
        function applyFieldDependenciesForSection(sectionId) {
            ...
            section.querySelectorAll('tr[data-field-row]').forEach((row) => {
                ...
                const normalizedParent = normalizeValueForDependency(parentField.value, parentKind);
        ...
        function recalculateFormulasForSection(sectionId) {
            ...
            section.querySelectorAll(`.formula-source[data-audit-section="${sectionId}"]`).forEach((input) => {
                ...
                if (kind === 'select') {
                    const validOpts = Array.from(input.options || []).filter((o) => o.value !== '').map((o) => o.value);
                    const idx = validOpts.indexOf(raw);
