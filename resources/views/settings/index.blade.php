<x-layouts.app>
    ...
    <section class="panel" style="display:flex; align-items:center; justify-content:space-between; gap:16px; flex-wrap:wrap;">
        <div>
            <h2 style="margin:0 0 4px; font-size:20px; font-weight:800; color:var(--ink);">{{ __('ui.settings.header.title') }}</h2>
    ...
    <style>
        ...
        .acc-item { background: #fff; border: 1px solid var(--paper-deep); border-radius: 14px; overflow: hidden; box-shadow: 0 4px 16px rgba(14,55,85,.05); }
        .acc-header {
        ...
        .acc-header:hover { background: var(--green-deep) !important; }
        .acc-icon {
            width: 36px; height: 36px; border-radius: 10px;
            display: grid; place-items: center;
            font-size: 16px; flex-shrink: 0;
        }
        .acc-icon-blue  { background: rgba(245,239,224,.15); color: var(--paper); }
        .acc-icon-green { background: rgba(245,239,224,.15); color: var(--paper); }
        .acc-icon-amber { background: rgba(245,239,224,.15); color: var(--gold-light); }
        .acc-title { flex: 1; }
        .acc-title strong { display: block; font-size: 15px; font-weight: 700; color: var(--paper) !important; }
        .acc-title span   { font-size: 12px; color: var(--green-light) !important; }
        .acc-chevron { font-size: 18px; color: var(--paper); transition: transform .2s; }
        .acc-item.open .acc-chevron { transform: rotate(180deg); }
        .acc-body { display: none; padding: 0 20px 20px; border-top: 1px solid #e8f0f7; }
        .acc-item.open .acc-body { display: block; }
        .acc-body table { margin-top: 14px; }
        .acc-body th { font-size: 11px; text-transform: uppercase; letter-spacing: .6px; color: var(--ink-mute); background: #f7fafc; padding: 9px 10px; }
        ...
        .acc-note { margin-top: 14px; padding: 10px 14px; background: var(--green-bg); border-radius: 10px; border-left: 3px solid var(--green-primary); font-size: 13px; color: #2a5a8a; }
        ...
        .perm-item { display: flex; align-items: center; gap: 8px; font-size: 13px; color: var(--ink); }
    ...
    <div class="accordion">
        ...
        {{-- 1. UŻYTKOWNICY --}}
        <div class="acc-item" id="acc-users">
            ...
            <div class="acc-body">
                @if($canManage)
                    ...
                    <form id="add-user-form" method="POST" action="{{ route('settings.user-store') }}" style="display:none; margin-top:14px; padding:12px; border:1px solid #dfeaf3; border-radius:10px; background:#f9fcff; flex-wrap:wrap; gap:10px; align-items:end;">
                        ...
                        <div>
                            <label style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute);">{{ __('ui.settings.users.first_name') }}</label>
                            <input type="text" name="first_name" value="{{ old('first_name') }}" required>
                        </div>
                        <div>
                            <label style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute);">{{ __('ui.settings.users.last_name') }}</label>
                            <input type="text" name="last_name" value="{{ old('last_name') }}" required>
                        </div>
                        <div>
                            <label style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute);">{{ __('ui.settings.users.columns.email') }}</label>
                            <input type="email" name="email" value="{{ old('email') }}" required>
                        </div>
                        <div>
                            <label style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute);">{{ __('ui.settings.users.phone') }}</label>
                            <input type="text" name="phone" value="{{ old('phone') }}">
                        </div>
                        <div>
                            <label style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute);">{{ __('ui.settings.users.password') }}</label>
                            <input type="password" name="password" required>
                        </div>
                        <div>
                            <label style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute);">{{ __('ui.settings.users.role_label') }}</label>
                        ...
                        <div style="display:flex; flex-direction:column; min-width:220px;">
                            <label style="font-size:12px; font-weight:700; color:var(--ink-mute);">Uprawnienia zakładek</label>
                                            ...
                                            <form id="user-access-form-{{ $user->id }}" method="POST" action="{{ route('settings.user-access', $user) }}">
                                                ...
                                                <div style="display:flex; gap:10px; align-items:flex-end; flex-wrap:wrap; margin-bottom:12px;">
                                                    <div style="flex:1; min-width:90px;">
                                                        <label for="first-name-{{ $user->id }}" style="display:block; font-size:11px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">{{ __('ui.settings.users.first_name') }}</label>
                                                    ...
                                                    <div style="flex:1.2; min-width:100px;">
                                                        <label for="last-name-{{ $user->id }}" style="display:block; font-size:11px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">{{ __('ui.settings.users.last_name') }}</label>
                                                    ...
                                                    <div style="flex:0.8; min-width:70px;">
                                                        <label for="short-name-{{ $user->id }}" style="display:block; font-size:11px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">{{ __('ui.settings.users.short_name') }}</label>
                                                    ...
                                                    <div style="flex:1.8; min-width:140px;">
                                                        <label for="email-{{ $user->id }}" style="display:block; font-size:11px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">{{ __('ui.settings.users.columns.email') }}</label>
                                                        <input id="email-{{ $user->id }}" type="email" name="email" value="{{ old('email', $user->email) }}" required style="width:100%;">
                                                    </div>
                                                    <div style="flex:1; min-width:90px;">
                                                        <label for="phone-{{ $user->id }}" style="display:block; font-size:11px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">{{ __('ui.settings.users.phone') }}</label>
                                                        <input id="phone-{{ $user->id }}" type="text" name="phone" value="{{ old('phone', $user->phone) }}" style="width:100%;">
                                                    </div>
                                                    <div style="flex:1.2; min-width:100px;">
                                                        <label for="password-{{ $user->id }}" style="display:block; font-size:11px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">{{ __('ui.settings.users.password') }}</label>
                                                    ...
                                                    <div style="flex:1; min-width:90px;">
                                                        <label for="role-{{ $user->id }}" style="display:block; font-size:11px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">{{ __('ui.settings.users.role_label') }}</label>
                    ...
                    <form id="add-company-form" method="POST" action="{{ route('settings.company-store') }}" enctype="multipart/form-data" style="display:{{ old('open_add_company') ? 'block' : 'none' }}; margin-top:14px; padding:16px; border:1px solid #dfeaf3; border-radius:10px; background:#f9fcff;">
                        ...
                        <div style="display:flex; align-items:flex-end; gap:8px; flex-wrap:wrap; padding-bottom:14px; margin-bottom:14px; border-bottom:1px solid #dfeaf3;">
                            <div style="flex:0 0 auto;">
                                <label for="new-company-nip" style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute);">NIP — znajdź firmę w GUS</label>
                            ...
                            <span id="nip-lookup-message" style="font-size:12px; color:var(--ink-mute); align-self:center;"></span>
                        </div>

                        {{-- Rząd 1: Nazwa + Skrót --}}
                        <div style="display:flex; gap:12px; margin-bottom:10px;">
                            <div style="flex:3; min-width:150px;">
                                <label for="new-company-name" style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">{{ __('ui.settings.clients.columns.company') }} *</label>
                                <input id="new-company-name" type="text" name="name" value="{{ old('name') }}" required style="width:100%;">
                            </div>
                            <div style="flex:1; min-width:80px;">
                                <label for="new-company-short-name" style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">Skrót</label>
                            ...
                            <div style="flex:1; min-width:100px;">
                                <label for="new-company-regon" style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">REGON</label>
                                <input id="new-company-regon" type="text" name="regon" value="{{ old('regon') }}" maxlength="14" placeholder="9 lub 14 cyfr" style="width:100%;">
                            </div>
                            <div style="flex:1; min-width:90px;">
                                <label for="new-company-krs" style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">KRS</label>
                        ...
                        <div style="display:flex; gap:12px; margin-bottom:10px; flex-wrap:wrap;">
                            <div style="flex:2; min-width:130px;">
                                <label for="new-company-street" style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">Ulica</label>
                                <input id="new-company-street" type="text" name="street" value="{{ old('street') }}" style="width:100%;">
                            </div>
                            <div style="flex:1; min-width:90px;">
                                <label for="new-company-postal" style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">Kod pocztowy</label>
                            ...
                            <div style="flex:1.5; min-width:110px;">
                                <label for="new-company-city" style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">Miejscowość</label>
                                <input id="new-company-city" type="text" name="city" value="{{ old('city') }}" style="width:100%;">
                            </div>
                            <div style="flex:1; min-width:100px;">
                                <label for="new-company-phone" style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">Telefon</label>
                                <input id="new-company-phone" type="text" name="phone" value="{{ old('phone') }}" style="width:100%;">
                            </div>
                            <div style="flex:1.5; min-width:130px;">
                                <label for="new-company-email" style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">Email</label>
                                <input id="new-company-email" type="email" name="email" value="{{ old('email') }}" style="width:100%;">
                            </div>
                            <div style="flex:1.5; min-width:130px;">
                                <label for="new-company-auditor" style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">Opiekun</label>
                        ...
                        <div style="display:flex; gap:12px; align-items:flex-end;">
                            <div style="flex:0 0 auto;">
                                <label style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">Logo firmy</label>
                                <input type="file" name="logo" accept="image/*" style="font-size:13px; display:block;">
                            </div>
                            <div style="flex:1; min-width:150px;">
                                <label for="new-company-description" style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">Opis</label>
                ...
                <div style="overflow-x:auto; margin-top:14px;">
                    <table id="companies-table" style="width:100%; border-collapse:collapse; font-size:13px;">
                        <thead id="companies-thead">
                            <tr style="background:#eaf2f8; color:var(--ink-mute);">
                        ...
                        <tbody id="companies-tbody">
                            @foreach($companies as $company)
                                <tr data-company-id="{{ $company->id }}"
                                    ...
                                    <td style="padding:8px 10px; font-weight:600; color:var(--ink);">{{ $company->name }}</td>
                                    <td style="padding:8px 10px; color:var(--ink-mute);">{{ $company->short_name ?: '—' }}</td>
                                    <td style="padding:8px 10px; color:var(--ink-mute); white-space:nowrap;">{{ $company->nip ?: '—' }}</td>
                                    <td style="padding:8px 10px; color:var(--ink-mute);">{{ $company->city ?: '—' }}</td>
                                    <td style="padding:8px 10px; color:var(--ink-mute); white-space:nowrap;">{{ $company->phone ?: '—' }}</td>
                                    <td style="padding:8px 10px; color:var(--ink-mute);">{{ $company->email ?: '—' }}</td>
                                                    ...
                                                    <tbody>
                                                        @foreach($company->assignedUsers as $u)
                                                            <tr style="border-bottom:1px solid #edf4fb;">
                                                                <td style="padding:5px 8px; color:var(--ink); font-weight:600; white-space:nowrap;">{{ trim(($u->first_name ?? '') . ' ' . ($u->last_name ?? '')) ?: $u->name }}</td>
                                                                <td style="padding:5px 8px; color:var(--ink-mute); white-space:nowrap;">{{ $u->short_name ?: '—' }}</td>
                                                                <td style="padding:5px 8px; color:var(--ink-mute);">{{ $u->position ?: '—' }}</td>
                                                                <td style="padding:5px 8px; color:var(--ink-mute);">{{ $u->email }}</td>
                                                                <td style="padding:5px 8px; color:var(--ink-mute); white-space:nowrap;">{{ $u->phone ?: '—' }}</td>
                                                            </tr>
                                                        @endforeach
                                                    </tbody>
                                                </table>
                                        ...
                                        <td colspan="8" style="padding:0;">
                                            <div class="permissions-panel">
                                                ...
                                                <form method="POST" action="{{ route('settings.company-update', $company) }}" enctype="multipart/form-data">
                                                    ...
                                                    <div style="display:flex; gap:12px; margin-bottom:10px; flex-wrap:wrap;">
                                                        <div style="flex:3; min-width:150px;">
                                                            <label style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">Nazwa *</label>
                                                        ...
                                                        <div style="flex:1; min-width:80px;">
                                                            <label style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">Skrót</label>
                                                        ...
                                                        <div style="flex:1.2; min-width:90px;">
                                                            <label style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">NIP</label>
                                                            <input type="text" name="nip" value="{{ old('open_company_editor_id') == $company->id ? old('nip', $company->nip) : $company->nip }}" style="width:100%;">
                                                        </div>
                                                        <div style="flex:1; min-width:100px;">
                                                            <label style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">REGON</label>
                                                        ...
                                                        <div style="flex:1; min-width:90px;">
                                                            <label style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">KRS</label>
                                                    ...
                                                    <div style="display:flex; gap:12px; margin-bottom:10px; flex-wrap:wrap;">
                                                        <div style="flex:2; min-width:130px;">
                                                            <label style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">Ulica</label>
                                                        ...
                                                        <div style="flex:1; min-width:90px;">
                                                            <label style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">Kod pocztowy</label>
                                                        ...
                                                        <div style="flex:1.5; min-width:110px;">
                                                            <label style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">Miejscowość</label>
                                                            <input type="text" name="city" value="{{ old('open_company_editor_id') == $company->id ? old('city', $company->city) : $company->city }}" style="width:100%;">
                                                        </div>
                                                        <div style="flex:1; min-width:100px;">
                                                            <label style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">Telefon</label>
                                                        ...
                                                        <div style="flex:1.5; min-width:130px;">
                                                            <label style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">Email</label>
                                                        ...
                                                        <div style="flex:1.5; min-width:130px;">
                                                            <label style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">Opiekun</label>
                                                    ...
                                                    <div style="display:flex; gap:12px; align-items:flex-end;">
                                                        ...
                                                        <div style="flex:1; min-width:150px;">
                                                            <label style="display:block; font-size:12px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">Opis</label>
                                                ...
                                                <div style="margin-top:16px; border-top:1px solid #dfeaf3; padding-top:14px;">
                                                    <strong style="font-size:13px; color:var(--ink); display:block; margin-bottom:10px;">Użytkownicy klienta</strong>
                                                    ...
                                                    <form id="company-contacts-form-{{ $company->id }}" method="POST" action="{{ route('settings.company-client-store', $company) }}" style="display:{{ old('open_company_editor_id') == $company->id && old('open_company_contacts') ? 'block' : 'none' }}; margin-bottom:12px; padding:12px; background:#f9fcff; border:1px solid #dfeaf3; border-radius:8px;">
                                                        ...
                                                        <div style="display:flex; gap:10px; align-items:flex-end; flex-wrap:wrap; margin-bottom:8px;">
                                                            <div style="flex:1; min-width:90px;">
                                                                <label style="display:block; font-size:11px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">Imię</label>
                                                            ...
                                                            <div style="flex:1.2; min-width:100px;">
                                                                <label style="display:block; font-size:11px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">Nazwisko</label>
                                                            ...
                                                            <div style="flex:0.8; min-width:70px;">
                                                                <label style="display:block; font-size:11px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">Skrót</label>
                                                            ...
                                                            <div style="flex:1.2; min-width:100px;">
                                                                <label style="display:block; font-size:11px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">Stanowisko</label>
                                                            ...
                                                            <div style="flex:1.8; min-width:140px;">
                                                                <label style="display:block; font-size:11px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">Email *</label>
                                                            ...
                                                            <div style="flex:1; min-width:90px;">
                                                                <label style="display:block; font-size:11px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">Telefon</label>
                                                            ...
                                                            <div style="flex:1.2; min-width:100px;">
                                                                <label style="display:block; font-size:11px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">Hasło</label>
                                                    ...
                                                    @if($company->assignedUsers->isNotEmpty())
                                                        <table style="width:100%; border-collapse:collapse; font-size:12px; margin-bottom:4px;">
                                                            ...
                                                            @foreach($company->assignedUsers as $assignedUser)
                                                                <tr style="border-bottom:1px solid #edf4fb;">
                                                                    <td style="padding:5px 8px; color:var(--ink); font-weight:600; white-space:nowrap;">{{ trim(($assignedUser->first_name ?? '') . ' ' . ($assignedUser->last_name ?? '')) ?: $assignedUser->name }}</td>
                                                                    <td style="padding:5px 8px; color:var(--ink-mute); white-space:nowrap;">{{ $assignedUser->short_name ?: '—' }}</td>
                                                                    <td style="padding:5px 8px; color:var(--ink-mute);">{{ $assignedUser->position ?: '—' }}</td>
                                                                    <td style="padding:5px 8px; color:var(--ink-mute);">{{ $assignedUser->email }}</td>
                                                                    <td style="padding:5px 8px; color:var(--ink-mute); white-space:nowrap;">{{ $assignedUser->phone ?: '—' }}</td>
                                                                ...
                                                                <tr id="assigned-user-editor-row-{{ $company->id }}-{{ $assignedUser->id }}" style="display:{{ old('open_assigned_user_id') == $assignedUser->id ? 'table-row' : 'none' }}; background:#f5faff;">
                                                                    <td colspan="6" style="padding:10px 8px;">
                                                                        ...
                                                                        <div style="display:flex; gap:10px; align-items:flex-end; flex-wrap:wrap; margin-bottom:8px;">
                                                                            ...
                                                                            <div style="flex:1.2; min-width:100px;">
                                                                                <label style="display:block; font-size:11px; font-weight:700; color:var(--ink-mute); margin-bottom:3px;">Nowe hasło</label>
    ...
    <script>
        ...
        // Auto-open accordion if session flash present
        @if(session('public_access_status'))
            document.addEventListener('DOMContentLoaded', function() {
                document.getElementById('acc-public-access')?.classList.add('open');
            });
        @endif
        @if(session('co2_settings_status'))
            document.addEventListener('DOMContentLoaded', function() {
                document.getElementById('acc-co2')?.classList.add('open');
            });
        @endif
...
@if($canManage)
    ...
    <div class="acc-item" id="acc-public-access">
        <button class="acc-header" onclick="toggleAcc('acc-public-access')">
            <div class="acc-icon acc-icon-blue">🌐</div>
            <div class="acc-title">
                <strong>Dostęp publiczny do zakładki Informacje</strong>
                <span>Kontroluje czy niezalogowani użytkownicy mogą wyświetlać stronę /informacje.</span>
            </div>
            <span class="acc-chevron">&#9660;</span>
        </button>
        <div class="acc-body">
        ...
        <div style="margin-top:10px; font-size:12px; color:var(--ink-mute); background:var(--green-bg); border-radius:6px; padding:10px 14px; border-left:3px solid var(--green-primary);">
    ...
    </div>{{-- /.acc-item acc-public-access --}}

    {{-- ── WSKAŹNIKI EMISJI CO₂ ── --}}
    <div class="acc-item" id="acc-co2">
        <button class="acc-header" onclick="toggleAcc('acc-co2')">
            <div class="acc-icon acc-icon-green">🌿</div>
            <div class="acc-title">
                <strong>Wskaźniki emisji CO₂ — energia elektryczna (KSE)</strong>
                <span>Wartości aktualizowane raz w roku po publikacji KOBiZE. Widoczne w kalkulatorze i stałych energetycznych.</span>
            </div>
            <span class="acc-chevron">&#9660;</span>
        </button>
        <div class="acc-body">
        ...
        <form method="POST" action="{{ route('settings.energy-indicators') }}" style="display:flex; flex-wrap:wrap; gap:14px; align-items:flex-end;">
            ...
            <div>
                <label style="display:block; font-size:12px; font-weight:700; color:#1a5c2e; margin-bottom:5px;">
                    ...
                    <span style="font-weight:400; color:var(--ink-mute);">(EU ETS, białe certyfikaty)</span>
                </label>
            ...
            <div>
                <label style="display:block; font-size:12px; font-weight:700; color:#1a5c2e; margin-bottom:5px;">
                    ...
                    <span style="font-weight:400; color:var(--ink-mute);">(CSR, ślad węglowy)</span>
                </label>
        ...
        <div style="margin-top:12px; font-size:12px; color:var(--ink-mute); background:#edf3f8; border-radius:8px; padding:10px 14px; border-left:3px solid var(--green-primary); line-height:1.7;">
            📥 Źródło: <a href="https://www.kobize.pl/pl/fileCategory/id/28/wskazniki-emisyjnosci" target="_blank" rel="noopener noreferrer" style="color:var(--green-primary);">KOBiZE — Wskaźniki emisyjności</a>
            · Publikacja zwykle w grudniu każdego roku · Plik:
            <a href="https://www.kobize.pl/uploads/materialy/materialy_do_pobrania/aktualnosci/2025/142_Wskazniki_emisyjnosci_2025.pdf" target="_blank" rel="noopener noreferrer" style="color:var(--green-primary);">142_Wskazniki_emisyjnosci_2025.pdf</a>
        </div>
                ...
                <tbody>
                    ...
                    <tr style="border-bottom:1px solid #d2e3f1; {{ $loop->even ? 'background:#f9fdfb;' : '' }}">
                        ...
                        <td style="padding:7px 12px;">
                            @if($h->source_url)
                                <a href="{{ $h->source_url }}" target="_blank" rel="noopener noreferrer" style="color:var(--green-primary); font-size:12px;">pobierz PDF</a>
                            @else
