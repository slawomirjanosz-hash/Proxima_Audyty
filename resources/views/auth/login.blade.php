...
<head>
    ...
    <style>
        ...
        .logo { width:42px; height:42px; border-radius:12px; background:linear-gradient(130deg, #1ba84a, #0e89d8); display:grid; place-items:center; color:#fff; font-weight:800; }
...
<body>
    <form class="card" method="POST" action="{{ route('login.store', [], false) }}">
        ...
        <div class="row">
            <label style="margin:0;"><input type="checkbox" name="remember"> Zapamietaj mnie</label>
        </div>

        <button type="submit">Zaloguj</button>

        <div class="hint">
            Testowe konta po seederze: admin@enesa.pl, auditor@enesa.pl, client@enesa.pl (haslo: password)
        </div>
    </form>
</body>
</html>
